 ...
 */
contract AOC is Context, ITRC20, ITRC20Metadata, Ownable, Pausable {
    ...
    string private constant _symbol = "AOC TRC20";
    ...
    event TRXFromContractTransferred(
        uint256 amount
    );
     ...
     */
    constructor () {
        _mint(_msgSender(), (10000 * 10**9 * 10**18)); //mint the initial total supply
    }
     ...
     * `amount`.
     */
    function burnFrom(address account, uint256 amount) public virtual onlyOwner whenNotPaused {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(!blacklisted[sender] || !blacklisted[recipient], "AOC: Cant't transfer, sender or recipient  is blacklisted");
        ...
        require(senderBalance >= amount, "AOC: transfer amount exceeds balance");
    ...
    function withdrawTRXFromContract(uint256 amount) external onlyOwner payable {
        require(amount <= address(this).balance, "withdrawTRXFromContract: withdraw amount exceeds TRX balance");        
        address payable _owner = payable(msg.sender);        
        _owner.transfer(amount);     
        emit TRXFromContractTransferred(amount);
    }
    ...
    function withdrawToken(address _tokenContract, uint256 _amount) external onlyOwner {
        ...
        ITRC20 tokenContract = ITRC20(_tokenContract);
